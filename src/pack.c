	...
	git_off_t *obj_offset)
{
	...
	while (git_array_size(chain) > 0) {
		...
		/* we only want to try to cache if we're not handing this buffer off to the user */
		more = git_array_size(chain) > 0;
		if (more && cache_add(&p->bases, obj, elem->base_key) < 0) {
			git__free(base.data);
		}
